...
if ! source "${scrDir}/global.sh"; then
    echo -e "\e[31mError: unable to source global.sh...\e[0m"
    exit 1
fi
...
while read -r lst; do
    ...
    if [ -w "${tgt}" ]; then
        # shellcheck disable=SC2154
        [ "${flg_DryRun}" -eq 1 ] || tar -xzf "${scrDir}/Source/arcs/${fnt}.tar.gz" -C "${tgt}/"
    else
        print_log -warn "not writable" "Extracting as root: ${tgt} "
        if [ "${flg_DryRun}" -ne 1 ]; then
            if ! sudo tar -xzf "${scrDir}/Source/arcs/${fnt}.tar.gz" -C "${tgt}/" 2>/dev/null; then
